...
| Recall changes behavior | Local deterministic and public OpenRouter Agent paths changed the same intent from READY draft to causal DENY | Show the real-model decision diff in the unedited video |
...
| Public repo and real history | Public repository exists; implementation is split into reviewable commits | Push the latest model/evidence commits after review |
