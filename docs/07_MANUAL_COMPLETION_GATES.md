...
complete. Stable Render HTTPS hosting and persistent restart behavior are also
complete. The remaining items require continuous recording, publication choices,
the owner's account, or wallet approval. They cannot be
...
| Real Agent behavior | **Passed on public Render: receipt-bound OpenRouter A/B across restart** | Schema 1.1 receipts are stored in both public Sibyl Agent runs; repeat in continuous video | Adjacent runtime health is called cryptographic run binding |
...
| Real fresh-session Sibyl run | **Public Render restart + schema 1.1 remote A/B passed; video missing** | One unedited capture of separate A/B processes, same action hash, exact causal DENY, receipt-bound successful remote generations, and visible commit | JSON alone is said to prove the restart/checkout |
...
| Public HTTPS deployment | **Passed: Render Starter, persistent disk, public HTTPS, restart persistence** | <https://proofops-memoryguard.onrender.com>, `/health/ready`, `/evidence`, and the public A/B run records | Blueprint or random Quick Tunnel URL is called durable hosting |
